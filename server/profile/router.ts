...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const allPosts = await ProfileCollection.findAll();
  ...
  async (req: Request, res: Response) => {
    const authorPosts = await ProfileCollection.findAllByUsername(req.query.author as string);
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const friendedUsersPosts = await ProfileCollection.findAllByFriendedUsers(currentUserId);
...
router.post(
  ...
  [
    userValidator.isUserLoggedIn,
    profileValidator.isValidPostContent,
    profileValidator.isPostPropertyComplete
  ],
  async (req: Request, res: Response) => {
    const userId = (req.session.userId as string) ?? ''; // Will not be an empty string since its validated in isUserLoggedIn
    const post = await ProfileCollection.addOne(userId, req.body.photo, req.body.caption, req.body.focusReflection, req.body.progressReflection);
...
router.delete(
  ...
  [
    userValidator.isUserLoggedIn,
    profileValidator.isPostExists,
    profileValidator.isValidPostModifier
  ],
  async (req: Request, res: Response) => {
    await ProfileCollection.deleteOne(req.params.postId);
    res.status(200).json({
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    const post = await ProfileCollection.updateOne(req.params.postId, req.body.photo);
    res.status(200).json({
