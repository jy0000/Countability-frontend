...
class ProfileCollection {
   ...
   */
  static async findOne(postId: Types.ObjectId | string): Promise<HydratedDocument<Profile>> {
    return ProfileModel.findOne({_id: postId}).populate('authorId');
  }
  ...
  static async findAll(): Promise<Array<HydratedDocument<Profile>>> {
    ...
    return ProfileModel.find({}).sort({dateModified: -1}).populate('authorId');
  }
  ...
  static async findAllByUsername(username: string): Promise<Array<HydratedDocument<Profile>>> {
    ...
    return ProfileModel.find({authorId: author._id}).sort({dateModified: -1}).populate('authorId');
  }
   ...
   * @return {Promise<HydratedDocument<Post>[]>} - An array of all of the posts
   */
  static async findAllByPostType(targetPostType: string): Promise<Array<HydratedDocument<Profile>>> {
    return ProfileModel.find({caption: targetPostType}).sort({dateModified: -1}).populate('authorId');
  }
   ...
   * @return {Promise<HydratedDocument<Post>[]>} - An array of all of the posts
   */
  static async findAllByFriendedUsers(currentUserId: string): Promise<Array<HydratedDocument<Profile>>> {
    ...
    const postsFromFriendedUsers = await ProfileModel.find({authorId: {$in: allFriendedUserIds}}).sort({dateModified: -1}).populate('authorId');
   ...
   */
  static async updateOne(postId: Types.ObjectId | string, photo: string): Promise<HydratedDocument<Profile>> {
    const post = await ProfileModel.findOne({_id: postId});
    post.photo = photo;
  ...
  static async deleteOne(postId: Types.ObjectId | string): Promise<boolean> {
    const post = await ProfileModel.deleteOne({_id: postId});
    return post !== null;
  }
  ...
  static async deleteMany(authorId: Types.ObjectId | string): Promise<void> {
    await ProfileModel.deleteMany({authorId});
  }
}
